using ComputerWorkFileImplement.Models;
...
namespace ComputerWorkFileImplement.Implements
{
    public class ComputerLogic : IComputerLogic
    {
        ...
        {
            Computer element = source.Computers.FirstOrDefault(rec => rec.ComputerName ==
           model.ComputerName && rec.Id != model.Id);
            if (element != null)
            {
           ...
           !model.ComputerComponents.ContainsKey(rec.ComponentId));
            // обновили количество у существующих записей
            var updateComponents = source.ComputerComponents.Where(rec => rec.ComputerId ==
           model.Id && model.ComputerComponents.ContainsKey(rec.ComponentId));
            ...
            // добавили новые
            int maxPCId = source.ComputerComponents.Count > 0 ?
           source.ComputerComponents.Max(rec => rec.Id) : 0;
